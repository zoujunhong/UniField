...
class DrivAerML(torch.utils.data.Dataset):
    def __init__(
        self,
        data_root='/data/home/zdhs0017/zoujunhong/data/DrivAerML/drivaer_data/point_cloud',
        num_points=1024,
        repeat=100,
        sample=True,
        force_root='/data/home/zdhs0017/zoujunhong/data/DrivAerML/drivaer_data/',
        index_offset=0,        # 如果 run_/force_mom_ 从1开始，设为 1
        return_lift=False,     # True 时也返回 cl
    ):
        self.data_root = data_root
        self.file_list = sorted(os.listdir(data_root))
        self.file_list = self.file_list[int(0.9*len(self.file_list)):]
        ...
        print('DrivAerML: load {} fields snapshot for training'.format(self.__len__()))
    ...
    def __getitem__(self, idx):
        ...
        # 读取点云
        path = osp.join(self.data_root, self.file_list[base_idx])
        data = np.loadtxt(path)
        data = torch.from_numpy(data).float()[:, [0, 1, 2, 3]]
        ...
        # 读取 cd / cl（允许 NaN）
        data_idx = int(self.file_list[base_idx].split('_')[-1].split('.')[0])
        cd, cl = self._read_cd_cl(data_idx)
        # cd = torch.tensor(cd, dtype=torch.float32)
        # cl = torch.tensor(cl, dtype=torch.float32)
        
        coeff = torch.tensor((cd,float('nan'),float('nan'),float('nan'),float('nan')), dtype=torch.float32)
        
        return data[:, :3], data[:, 3], torch.tensor([38.89, 0]), torch.tensor(0).long(), coeff
